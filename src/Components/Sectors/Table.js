export default function TableSector({ dataSet, setCityOnChart }) {
  ...
  for (let x in dataSet) {
    rows.push(
      ...
      >
        ...
        <span
          className={`bg-black font-bold text-center ${getAQSCategory(
            dataSet[x].val
          )}`}
        >
          {dataSet[x].val.toFixed(2)}
        </span>
        <span className={`text-sm font-light`}>
          {getUpdatedTime(dataSet[x].time)}
        </span>
      </div>
    );
  }
  ...
  return (
    <div onClick={setCityOnChart}>
      {rows.length > 0 && (
        <div
          className={`grid grid-cols-3 gap-2 text-white bg-coolGray-600 font-serif`}
        >
